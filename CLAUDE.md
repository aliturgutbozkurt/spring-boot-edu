...
- MongoDB (Boot 4): connection settings are `spring.mongodb.*` (the old `spring.data.mongodb.uri/database/...` are deprecated); Spring Data settings stay under `spring.data.mongodb.*`. Set `spring.data.mongodb.representation.big-decimal: decimal128`, otherwise `BigDecimal` is stored as a string and `$sum`/`$avg` do not work. Boot registers no MongoDB transaction manager — declare a `MongoTransactionManager` bean.
