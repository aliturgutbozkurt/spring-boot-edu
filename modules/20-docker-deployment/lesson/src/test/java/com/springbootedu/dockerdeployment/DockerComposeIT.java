...
/**
 * Lessons 3.2 and 3.6 — builds the Dockerfile (via compose.yaml) after "package" and starts app + database.
 */
class DockerComposeIT {

    static final ComposeContainer SYSTEM = new ComposeContainer(new File("../compose.yaml"))
            .withBuild(true)
            .withEnv("BOOKSTORE_SHOP_NAME", "Compose IT")
    ...
    @Test
    void theContainerServesTheApi() throws Exception {
        assertThat(get("/api/books")).contains("Effective Java");
    }

    @Test
    void theEnvironmentConfiguresTheApplication() throws Exception {
        assertThat(get("/api/shop")).contains("\"name\":\"Compose IT\"");
    }
