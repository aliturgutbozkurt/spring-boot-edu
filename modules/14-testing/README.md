# 14 · Spring Boot'ta Test / Testing Spring Boot Applications

🇹🇷 [Ders notları](docs/tr/ders.md) ([PDF](docs/tr/ders.pdf)) · [Ödevler](docs/tr/odevler.md) ([PDF](docs/tr/odevler.pdf))
🇬🇧 [Lesson notes](docs/en/lesson.md) ([PDF](docs/en/lesson.pdf)) · [Exercises](docs/en/exercises.md) ([PDF](docs/en/exercises.pdf))

## 🇹🇷 Bu modülde

Bu modülde ders, **testlerin kendisidir**: `./mvnw -pl modules/14-testing/lesson verify`

- Test piramidi: unit (JUnit, Mockito) → slice (`@WebMvcTest`, `@DataJpaTest`, `@JsonTest`) → entegrasyon (`@SpringBootTest`)
- `MockMvcTester`, `RestTestClient`, `@MockitoBean`, `@MockitoSpyBean`
- Testcontainers + `@ServiceConnection` + reuse, test verisi için builder'lar
- ArchUnit ile mimari kuralları

## 🇬🇧 In this module

In this module the lesson **is the tests**: `./mvnw -pl modules/14-testing/lesson verify`

- The test pyramid: unit (JUnit, Mockito) → slice (`@WebMvcTest`, `@DataJpaTest`, `@JsonTest`) → integration (`@SpringBootTest`)
- `MockMvcTester`, `RestTestClient`, `@MockitoBean`, `@MockitoSpyBean`
- Testcontainers + `@ServiceConnection` + reuse, builders for test data
- Architecture rules with ArchUnit

## Çalıştırma / How to run

Ön koşul / Prerequisite: JDK 27, Docker — `export JAVA_HOME=$(/usr/libexec/java_home -v 27)`

```bash
# Ders kodunu çalıştır — gereken altyapı Docker Compose ile otomatik başlar
# Run the lesson — required infrastructure starts automatically via Docker Compose
./mvnw -pl modules/14-testing/lesson spring-boot:run

# Altyapı zaten çalışıyorsa (docker compose --profile ... up -d) / If infrastructure is already running
./mvnw -pl modules/14-testing/lesson spring-boot:run -Dspring-boot.run.arguments=--spring.docker.compose.enabled=false

# Testler / Tests (Testcontainers)
./mvnw -pl modules/14-testing/lesson verify

# Ödevler / Exercises (kırmızı başlar / start red)
./mvnw -Pexercises -pl modules/14-testing/exercise test
```

HTTP örnekleri / HTTP examples: [requests.http](requests.http)

## Yapı / Layout

| Klasör / Folder | İçerik / Content |
|---|---|
| `lesson/` | Çalışan örnekler + testler / Runnable examples + tests |
| `exercise/` | TODO'lu başlangıç kodu / Starter code with TODOs |
| `solution/` | Referans çözüm / Reference solution |
| `docs/` | Ders ve ödev dokümanları (MD + PDF) / Lesson & exercise docs |
